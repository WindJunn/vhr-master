<template>
  <el-container v-loading="loading" class="post-article">
    ...
    <div v-text="link"></div>
     <ul>
      <li v-for="item in fileLists" :key="item">文件链接：{{item}}</li>
    </ul>
    <el-upload
      class="upload-demo"
      action="/article/uploadVideo"
...
export default {
  ...
  methods: {
    fileUploadSuccess(response, file, fileList) {
      var _this = this;
      if (response) {
        _this.$message({ type: response.status, message: response.msg });
  ...
  data() {
    return {
      link:
        '<div align="center"><video controls="true" width="400" src="此处替换为视频文件链接"></video></div>',
      link1:
        '<div align="center"><img width="400" height="300" src="此处替换为图片文件链接" alt="此处替换为图片名" /></div>',
      fileList: [],
      fileLists: [],
      categories: [],
