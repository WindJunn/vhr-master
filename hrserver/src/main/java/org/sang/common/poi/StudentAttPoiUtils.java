 ...
 */
public class StudentAttPoiUtils {
    ...
    public static ResponseEntity<byte[]> exportEmp2Excel(List<Student> emps) {
        ...
        try {
            ...
            //创建Excel表单
            HSSFSheet sheet = workbook.createSheet("学员考勤信息表");
            //创建日期显示格式
            ...
            headers.setContentDispositionFormData("attachment",
                    new String("学员考勤表.xls".getBytes("UTF-8"), "iso-8859-1"));
    ...
    public static List<Attendance> importAtt2List(MultipartFile file,
                                                  List<Student> students,
                                                  List<Attname> attnames) {
                    ...
                    for (int k = 0; k < physicalNumberOfCells; k++) {
                        ...
                        switch (cell.getCellTypeEnum()) {
                                ...
                                switch (k) {
                                    case 4:
                                        ...
                                        for (int m = 0; m < students.size(); m++) {
                                            if (cellValue.equals(students.get(m).getIdCard())){
                                                id = students.get(m).getId();
                                                break;
                                            }
                                        }
                                        att.setSid(id);
                                    ...
                                    case 12:
                                        ...
                                        for (int n = 0; n < attnames.size(); n++) {
                                            if (cellValue.equals(attnames.get(n).getName())){
                                                id1 = attnames.get(n).getId();
                                            }
                            ...
                            default: {
                                switch (k) {
                                    case 11:
                                        att.setAtime(cell.getDateCellValue());
                                        break;
                                }
                            }
                            break;
